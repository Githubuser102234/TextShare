...
// NEW: Define a variable to store the blocked words list
let blockedWords = [];

// NEW: Asynchronously fetch the blocked words list when the page loads
async function fetchBlockedWords() {
    try {
        const response = await fetch('blocked_text.txt');
        if (!response.ok) {
            throw new Error('Failed to fetch blocked words list.');
        }
        const text = await response.text();
        // Split the text by lines and filter out any empty entries
        blockedWords = text.split('\n').map(word => word.trim().toLowerCase()).filter(word => word.length > 0);
        console.log("Blocked words loaded successfully.");
    } catch (e) {
        console.error("Error fetching blocked words:", e);
    }
}

// NEW: Call the function to fetch the blocked words list
fetchBlockedWords();

// NEW: Function to check if the text contains any blocked words
...
submitBtn.addEventListener('click', async () => {
    ...
    // NEW: Check if the text contains any blocked words before proceeding
